...
bool AudioRecoder::init(QString& status)
{
    //打开设备
...
void AudioRecoder::run()
{
    ...
    while (!m_stop)
    {
        ...
        rc = snd_pcm_readi(m_handle, data, m_frameSize);
//        qDebug() <<"cc:" << QTime::currentTime().msec();
        if (rc == -EPIPE)
        {
              /* EPIPE means overrun */
        ...
        SocketManage::getInstance()->sendVoiceData(audioData);
